...
This document defines the supported alpha path that AgentFirewall intends to keep stable through `0.1.0a1`.

For release-readiness status and the final validation checklist, see [RELEASE_READINESS.md](./RELEASE_READINESS.md).
...
Use `agentfirewall.approval` for the documented alpha approval helper path:
...
Lower-level helpers under modules such as `agentfirewall.enforcers`, `agentfirewall.audit`, `agentfirewall.policy_packs`, and `agentfirewall.integrations.langgraph` are still useful, but they are advanced usage rather than the primary supported alpha entrypoint.
...
The documented alpha path for local demos, evals, and simple integrations is `StaticApprovalHandler`:
...
Custom approval callbacks are still supported, but `StaticApprovalHandler` is the recommended documented alpha helper because it is deterministic and easy to verify in demos and evals.
...
The trial runner and eval runner both emit ordered traces. For guarded shell, HTTP, and file events, those traces now include `runtime_context` metadata such as the originating `tool_name` and `tool_call_id`.
...
That makes it easier to see whether a workflow is failing at the LangGraph middleware layer, a guarded tool boundary, or a specific tool chain.

For `log-only` flows, trace entries also preserve `decision_metadata.original_action` so a user can see whether a step would have been reviewed or blocked before turning on enforcement.

The packaged eval suite currently covers 17 task-oriented local cases, and the local trial runner covers 9 local workflows built to look more like real user tasks instead of isolated tool calls.
...
- a fully frozen public API outside the supported modules listed above
