"""Local MCP client preview eval runner for AgentFirewall."""
...
from ..mcp import create_client_bundle
...
def _default_case_resource() -> Traversable:
    return resources.files("agentfirewall.evals").joinpath("cases/mcp_client_cases.json")


def load_mcp_client_eval_cases(path: str | Traversable | None = None) -> list[McpClientEvalCase]:
    if path is None:
    ...
    return [McpClientEvalCase(**case) for case in payload]


def _make_approval_handler(case: McpClientEvalCase) -> ApprovalHandler | None:
...
def run_mcp_client_eval_case(case: McpClientEvalCase) -> EvaluationResult:
...
def run_mcp_client_eval_suite(path: str | Traversable | None = None) -> EvaluationSummary:
    cases = load_mcp_client_eval_cases(path)
    return EvaluationSummary(results=[run_mcp_client_eval_case(case) for case in cases])
